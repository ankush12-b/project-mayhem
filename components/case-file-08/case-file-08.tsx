...
function useMusicEngine() {
  ...
  function start() {
    ...
    function pulse() {
      ...
      const tid = setTimeout(pulse, 2800 + Math.random() * 1200);
    ...
    function shim() {
      ...
      const tid = setTimeout(shim, 3000 + Math.random() * 5000);
      r.current.timers.push(tid);
    }
    setTimeout(shim, 1500);
    ...
    function motif() {
      ...
      const tid = setTimeout(motif, 1600 + Math.random() * 2800);
      r.current.timers.push(tid);
    }
    setTimeout(motif, 800);
...
function Dialogue({ sceneKey, onComplete }) {
  ...
  useEffect(() => { setTyping(true); setVisible(false); const t = setTimeout(() => { setTyping(false); setTimeout(() => setVisible(true), 60); }, 700 + Math.random() * 400); return () => clearTimeout(t); }, [cur]);
  function advance() { if (cur < lines.length - 1) setCur(c => c + 1); else onComplete(); }
  const line = lines[cur]; if (!line) return null;
  const ch = CHARS[line.c]; const isU = line.c === "USER"; const isSys = line.c === "SYSTEM";
  return (
    <div className="dlg fade-in">
      <div className="dlg-hist">
        {lines.slice(0, cur).map((l, i) => {
          const iu = l.c === "USER";
          return (<div key={i} className={`dlg-ghost-row ${iu?"dlg-gr":"dlg-gl"}`}><div className="dlg-ghost">{l.t}</div></div>);
        })}
      </div>
      <div className={`dlg-active ${isU?"dlg-right":"dlg-left"}`}>
        {!isU && <div className="dlg-av" style={{ borderColor: ch.color, borderRadius: isSys ? "4px" : "50%" }}><span>{ch.avatar}</span></div>}
        ...
        {isU && <div className="dlg-av" style={{ borderColor: ch.color }}><span>{ch.avatar}</span></div>}
      </div>
      <div className="dlg-foot">
        <div className="dlg-dots">{lines.map((_,i) => <span key={i} className={`dlg-pdot ${i===cur?"a":i<cur?"d":""}`}/>)}</div>
        <button className="dlg-next" onClick={advance} disabled={typing}>{cur < lines.length - 1 ? "CONTINUE ›" : "PROCEED ›"}</button>
      </div>
    </div>
  );
}
...
function Cooldown({ seconds, reason, onDone }) {
  ...
  useEffect(() => {
    const iv = setInterval(() => setRem(r => { if (r <= 1) { clearInterval(iv); onDone(); return 0; } return r - 1; }), 1000);
...
function Shell({ index, title, prompt, hints, children }) {
  ...
  function useHint(i) {
    ...
    setUsed(u => [...u, i]); setHintIdx(i); setCooldown({ s: 60, r: "hint" });
  }
  const childWithProps = children ? (Array.isArray(children) ? children : [children]).map((c, i) =>
    c && typeof c === "object" ? { ...c, props: { ...c.props, wrong, locked } } : c
  ) : null;
  return (
    <div className="pz fade-in" style={{ position: "relative" }}>
      {cooldown && <Cooldown key={cooldown.r + cooldown.s} seconds={cooldown.s} reason={cooldown.r} onDone={() => setCooldown(null)}/>}
          ...
          {hints.map((h, i) => (
            <div key={i} className="hint-item">
              {used.includes(i)
                ...
                : <button className="hint-btn" onClick={() => useHint(i)} disabled={locked || (i > 0 && !used.includes(i-1))}>
...
function P1({ wrong, locked, onSolve }) {
  ...
  const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function submit(e) {
    e.preventDefault(); if (locked) return;
    const c = inp.trim().toLowerCase();
    if (["october","14 october 1972","14-oct-1972"].includes(c)) { setSt({ok:true,msg:"Confirmed — OCTOBER is the non-anagram. Transmission date verified."}); setTimeout(onSolve, 700); }
    else { setSt({ok:false,msg:"Not quite. Check which decoded word uses a completely different set of letters from the others."}); wrong(); }
  }
  return (<>
    ...
    <div className="p1-log">{P1_LINES.map((l,i)=>(
      <div key={i} className={`p1-entry ${open===i?"open":""}`} onClick={()=>setOpen(open===i?null:i)}>
        <span className="p1-time">[{l.time}]</span><span className="p1-raw">{l.raw}</span>
...
const STAFF = [
  ...
  {id:4,name:"E. Hartley",dept:"Security",clr:"A",entry:"21:30",exit:"21:15",flag:true},
  {id:5,name:"T. Nakamura",dept:"Chemistry",clr:"B",entry:"22:10",exit:"05:55",flag:false},
  {id:6,name:"Dr. K. Adeyemi",dept:"Neuroscience",clr:"D",entry:"21:45",exit:"04:30",flag:true},
  ...
  {id:9,name:"4. Brennan",dept:"Administration",clr:"C",entry:"22:15",exit:"05:45",flag:true},
...
function P2({ wrong, locked, onSolve }) {
  const [sel, setSel] = useState([]); const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function toggle(id){setSel(s=>s.includes(id)?s.filter(x=>x!==id):[...s,id]);}
  function submit(e){
    e.preventDefault(); if(locked) return;
    if(inp.trim().toUpperCase().replace(/\s/g,"") === "EK4"){setSt({ok:true,msg:"Three contradictions confirmed. Code EK4 verified."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Incorrect. Order matters — read initials as they appear in the ledger, top to bottom."});wrong();}
  }
  return (<>
    ...
    <div className="p2-tbl">
      ...
      {STAFF.map(s=>(
        <div key={s.id} className={`p2-row ${sel.includes(s.id)?"flagged":""}`} onClick={()=>toggle(s.id)}>
          <span>{s.id}</span><span>{s.name}</span><span>{s.dept}</span><span>{s.clr}</span><span>{s.entry}</span><span>{s.exit}</span><span>{sel.includes(s.id)?"⚑":"○"}</span>
        </div>))}
    </div>
    {sel.length>0&&<div className="p2-sel">Flagged: {sel.map(id=>STAFF.find(s=>s.id===id)?.name).join(", ")}</div>}
    <form className="row" onSubmit={submit}>
...
function P3({ wrong, locked, onSolve }) {
  const [rev, setRev] = useState({}); const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function submit(e){
    e.preventDefault(); if(locked) return;
    if(inp.trim().toUpperCase()==="DELTA"){setSt({ok:true,msg:"Codename confirmed — DELTA."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Not quite. Skip the impossible compound, then read the remaining five letters in written order."});wrong();}
  }
  return (<>
    ...
    <div className="p3-grid">{P3_COMPOUNDS.map(c=>(
      <div key={c.id} className={`p3-c ${rev[c.id]?"open":""} ${c.impossible?"imp":""}`} onClick={()=>setRev(r=>({...r,[c.id]:!r[c.id]}))}>
        ...
        {rev[c.id]&&<div className="p3-det">
          <div className="p3-bk">{c.breakdown}</div>
          <div className="p3-sm">Σ = {c.sum}</div>
          {c.impossible?<div className="p3-no">⚠ IMPOSSIBLE — discard</div>:<div className="p3-ltr">→ {c.letter}</div>}
        </div>}
      </div>))}</div>
    <form className="row" onSubmit={submit}>
...
function P4({ wrong, locked, onSolve }) {
  const [showGrid, setShowGrid] = useState(false); const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function submit(e){
    e.preventDefault(); if(locked) return;
    const c = inp.trim().replace(/\s/g,"");
    if(c==="7294"||c==="7-2-9-4"){setSt({ok:true,msg:"PIN confirmed — 7294."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Incorrect. Recover the corrupted cell first using the mirror rule, then average all four scan lines."});wrong();}
  }
  return (<>
    ...
    {showGrid&&<div className="p4-gw">
      <div className="p4-hrow"><span className="p4-rc">r\c</span>{[0,1,2,3,4,5,6,7].map(c=><span key={c} className="p4-ch">{c}</span>)}</div>
      {THE_GRID.map((row,r)=>(
        <div key={r} className="p4-row2"><span className="p4-rh">{r}</span>
          {row.map((cell,c)=><span key={c} className={`p4-cell ${cell==="?"?"cor":""}`}>{cell}</span>)}
        </div>))}
    </div>}
    <div className="p4-sl">
...
const WITNESSES = [
  {id:"W1",name:"Subject 001",time:"23:00",statements:[
    {id:"W1a",text:"I entered the cabinet at exactly 22:45.",true:true},{id:"W1b",text:"The cabinet door closed automatically behind me.",true:true},
...
function P5({ wrong, locked, onSolve }) {
  const [openW, setOpenW] = useState(null); const [flagged, setFlagged] = useState({}); const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function toggleFlag(wid, sid){setFlagged(f=>({...f,[`${wid}-${sid}`]:!f[`${wid}-${sid}`]}));}
  function submit(e){
    e.preventDefault(); if(locked) return;
    const c=inp.trim().toLowerCase().replace(/\s+/g," ");
    if(["the null room","null room"].includes(c)){setSt({ok:true,msg:"Location confirmed — THE NULL ROOM."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Not quite. Find one lie per witness. The key word in each of the first three witnesses' true statements (in witness order) gives you the answer."});wrong();}
  }
  return (<>
    ...
    <div className="p5-ws">{WITNESSES.map(w=>(
      <div key={w.id} className="p5-w">
        <button className="p5-wh" onClick={()=>setOpenW(openW===w.id?null:w.id)}>
          <span className="p5-wid">{w.name}</span><span className="p5-wt">Filed {w.time}</span><span>{openW===w.id?"▲":"▼"}</span>
        </button>
        {openW===w.id&&<div className="p5-stmts">{w.statements.map(s=>(
...
function P6({ wrong, locked, onSolve }) {
  const [visited, setVisited] = useState([]); const [letters, setLetters] = useState([]); const [cur, setCur] = useState("A-1"); const [inp, setInp] = useState(""); const [manual, setManual] = useState(""); const [st, setSt] = useState(null);
  function visit(code){
    const e=ARCHIVE[code]; if(!e){setSt({ok:false,msg:`No entry: ${code}`});return;}
    setVisited(v=>[...v,code]); setLetters(l=>[...l,e.letter]); setCur(e.next||"END");
  }
  function jumpTo(e){e.preventDefault();const c=manual.trim().toUpperCase();if(!ARCHIVE[c]){setSt({ok:false,msg:`No entry: ${c}`});return;}visit(c);setManual("");}
  function reset(){setVisited([]);setLetters([]);setCur("A-1");setSt(null);}
  function submit(e){
    e.preventDefault(); if(locked) return;
    const c=inp.trim().toUpperCase().replace(/[-\s]/g,"");
    if(c==="NULLGATE"){setSt({ok:true,msg:"Project name confirmed — NULL-GATE."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Incorrect. The valid chain has exactly 8 stops. Dead ends loop — if you've revisited a node, backtrack."});wrong();}
  }
  ...
  return (<>
    ...
    <div className="p6-term">
      ...
      {cur!=="END"&&entry&&<div className="p6-cur">
        <div className="p6-nd">CURRENT: {visited[visited.length-1]||"A-1"} — letter "{entry.letter}"</div>
        <div className="p6-et">{entry.text}</div>
        {entry.next?<button className="p6-fl" onClick={()=>visit(entry.next)} disabled={locked}>FOLLOW → {entry.next}</button>
...
const ROOMS = [
  {id:1,label:"Wet Lab",adj:[2,5]},{id:2,label:"Electrical Vault",adj:[1,3,6]},{id:3,label:"Server Room",adj:[2,4]},
  {id:4,label:"High-Voltage Bay",adj:[3,5,8]},{id:5,label:"Chemical Store",adj:[1,4,6]},{id:6,label:"Observation Deck",adj:[2,5,7]},
  {id:7,label:"Cabinet Chamber",adj:[6,8,9]},{id:8,label:"Cryogenic Store",adj:[4,7,9]},{id:9,label:"Medical Bay",adj:[7,8,10]},
  {id:10,label:"Corridor",adj:[9,11,12]},{id:11,label:"Pressure Chamber",adj:[10,12]},{id:12,label:"Director's Office",adj:[10,11]},
];
...
function P7({ wrong, locked, onSolve }) {
  const [flagV, setFlagV] = useState([]); const [corr, setCorr] = useState({2:"",4:"",8:""}); const [show, setShow] = useState(false); const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function toggleV(pair){const k=pair.join("-");setFlagV(f=>f.includes(k)?f.filter(x=>x!==k):[...f,k]);}
  function setC(r,v){setCorr(c=>({...c,[r]:v.toUpperCase().slice(0,1)}));}
  function submit(e){
    e.preventDefault(); if(locked) return;
    if(inp.trim().toUpperCase()==="WATCHER"){setSt({ok:true,msg:"Blueprint cipher resolved — WATCHER."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Not correct. Find all three violations, correct those legend entries, then read rooms 1–7 in sequence."});wrong();}
  }
  return (<>
    ...
    {show&&<div className="p7-w">
      ...
      <div className="p7-rooms">{ROOMS.map(r=>(
      ...
      <div className="p7-vs">
        ...
        {VIOLATIONS.map(pair=>{const k=pair.join("-");const r1=ROOMS.find(r=>r.id===pair[0]);const r2=ROOMS.find(r=>r.id===pair[1]);return(
          <button key={k} className={`p7-vp ${flagV.includes(k)?"fl":""}`} onClick={()=>toggleV(pair)}>
            ⚑ Room {pair[0]} ({r1.label}) ↔ Room {pair[1]} ({r2.label})
          </button>);})}
      </div>
      {flagV.length===3&&<div className="p7-corr">
        ...
        {[2,4,8].map(r=>(
          <div key={r} className="p7-cr">
            ...
            <input className="p7-ci" maxLength={1} value={corr[r]} onChange={e=>setC(r,e.target.value)} disabled={locked}/>
          </div>))}
      </div>}
    </div>}
    <form className="row" onSubmit={submit}>
...
function P8({ wrong, locked, onSolve }) {
  const [openS, setOpenS] = useState(null); const [ans, setAns] = useState(["","","",""]); const [inp, setInp] = useState(""); const [st, setSt] = useState(null);
  function setA(i,v){const a=[...ans];a[i]=v.slice(0,2);setAns(a);}
  function submit(e){
    e.preventDefault(); if(locked) return;
    if(inp.trim().toUpperCase()==="ZERO"){setSt({ok:true,msg:"Sequence anomaly confirmed — ZERO."});setTimeout(onSolve,700);}
    else{setSt({ok:false,msg:"Not correct. For each sequence: find the rule, determine the correct symbol at index 5, look it up in the table."});wrong();}
  }
  return (<>
    ...
    <div className="p8-st">
      ...
      <div className="p8-stg">{Object.entries(SYM_MAP).map(([s,l])=>(
        <div key={s} className="p8-stc"><span className="p8-sy">{s}</span><span className="p8-lt">{l}</span></div>))}
      </div>
    </div>
    <div className="p8-seqs">{P8_SEQS.map((sq,i)=>(
      <div key={sq.id} className="p8-sc">
        <button className="p8-sh" onClick={()=>setOpenS(openS===sq.id?null:sq.id)}>
          <span>SEQUENCE {sq.id}</span><span className="p8-sr">{sq.rule}</span><span>{openS===sq.id?"▲":"▼"}</span>
        </button>
        {openS===sq.id&&<div className="p8-sb">
          <div className="p8-syms">{sq.seq.map((s,j)=>(
            <span key={j} className={`p8-sym ${j===5?"intr":""}`}>
              <span className="p8-si">{j}</span><span className="p8-sv">{s}</span>
            </span>))}
          </div>
...
function P9({ wrong, locked, onSolve }) {
  const [vals, setVals] = useState(Array(8).fill("")); const [st, setSt] = useState(null);
  function setV(i,v){const a=[...vals];a[i]=v;setVals(a);}
  function submit(e){
    e.preventDefault(); if(locked) return;
    const bad = DOSSIER.map((b,i)=>vals[i].trim().toUpperCase().replace(/\s+/g," ")!==b.answer?b.id:null).filter(Boolean);
    if(bad.length===0){setSt({ok:true,msg:"ALL FIELDS CONFIRMED. Final dossier unlocked."});setTimeout(onSolve,900);}
    else{setSt({ok:false,msg:`Fields ${bad.join(", ")} incorrect. Two fields require details spoken aloud — not found in any file.`});wrong();}
  }
  return (<>
...
function Header({ stage, elapsed, onRestart, muted, onToggle }) {
  ...
  const h = Math.floor(elapsed/3600), m = Math.floor((elapsed%3600)/60), s2 = elapsed%60;
  const fmt = n => String(n).padStart(2,"0");
  return (
    <div className="hd">
      <div className="hd-l">
        <span style={{fontSize:16}}>🗂</span>
        <div><div className="hd-title">BROKEN DECK</div><div className="hd-sub">PUZZLE {Math.min(done+1,9)} / 9</div></div>
      </div>
      <div className="hd-r">
        <div className="hd-timer">⏱ {fmt(h)}:{fmt(m)}:{fmt(s2)}</div>
        <div className="hd-dots">{Array(9).fill(0).map((_,i)=><span key={i} className={`hd-dot ${i<done?"done":i===done?"cur":""}`}/>)}</div>
...
export default function App() {
  ...
  function goNext() {
    ...
    setTimeout(() => {
      ...
      else { if (SEQ[next].type==="puzzle" && !timerOn) setTimerOn(true); setStage(next); }
      setTrans(false);
    }, 300);
  }
  ...
  return (
    <div className="root">
      ...
      {phase==="game" && (<>
        ...
        <div className="main">
          <div className="stage" style={{opacity:trans?0:1,transition:"opacity .3s"}}>
            {step?.type==="cutscene" && <Dialogue key={step.key} sceneKey={step.key} onComplete={goNext}/>}
            {step?.type==="puzzle" && (() => {
      ...
      {phase==="finale" && (<>
        ...
        <div className="main"><div className="stage"><Finale elapsed={elapsed} onRestart={restart}/></div></div>
      </>)}
    </div>
  );
}
